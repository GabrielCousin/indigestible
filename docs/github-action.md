...
- 🕐 **Scheduled Execution**: Runs automatically every Monday at 8AM Paris time
- 🚀 **Manual Trigger**: Can be triggered manually via workflow_dispatch
- 📝 **Issue Creation**: Creates a formatted GitHub issue with all newsletter content
- 📦 **Artifact Upload**: Saves raw output files as workflow artifacts
- 🏷️ **Auto-labeling**: Issues are tagged with `newsletter` and `automated` labels
- 📊 **Table of Contents**: Generated issue includes a navigable TOC
...
- **Call-to-action removal** - Subscribe buttons, follow requests, promotional messages filtered out
- **Clean formatting** - Links cleaned, tracking parameters removed, footer content excluded
...
```markdown
# 🤖 Weekly Newsletter Digest (AI Summary)
...
```javascript
title: `📰 Newsletter Digest - ${dateStr}`,
```
...
- OpenAI API key is stored as a GitHub secret (never exposed in logs)
